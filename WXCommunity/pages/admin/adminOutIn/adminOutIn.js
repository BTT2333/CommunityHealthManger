...
Page({
  ...
  getDataAll: function () {
    var that = this;
    wx.request({
      url: 'http://localhost:8080/queryOutRegisterAll',
      success(res) {
        // console.log(res);
        that.setData({
          dataList: res.data.outRegister
        })
  ...
  getDataByDate: function (starttime) {
    console.log(starttime.substring(0, 10))
    console.log(starttime.substring(11, 16))
    var that = this;
    wx.request({
      url: 'http://localhost:8080/queryOutRegisterByDate',
      method: "POST",
      data: {
        date: starttime.substring(0, 10),
        time: starttime.substring(11, 16)
      },
      header: {
